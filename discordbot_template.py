...
async def miaou(ctx):
    try:
        ...
        vc.play(discord.FFmpegPCMAudio("/root/cat.mp3"), after=lambda e: print('done', e))
...
async def maisoui(ctx):
    try:
        channel = ctx.message.author.voice.channel
        vc = await channel.connect()
        vc.play(discord.FFmpegPCMAudio("/root/maisoui.mp3"), after=lambda e: print('done', e))
        await asyncio.sleep(3)
        await vc.disconnect()
    except Exception as e:
        print(str(e))
